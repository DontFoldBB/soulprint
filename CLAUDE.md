...
- **Current deployed Soulprint:** `0x6876041cc67f9cd1b11e6e1827b13f3622d256e5`
  (redeployed 2026-05-26 — adds **Prepaid Evolution Fuel**: per-token `evoBalance` mapping +
  `topUpEvolution(tokenId)` payable + `evolutionFuel(tokenId)` view + `EvolutionPaused` event.
  `INITIAL_FUEL_GRANT = 0.4 STT` (1 evolution included with mint), `EVOLUTION_COST = 0.4 STT`.
  `withdraw` gated by new `availableForWithdraw` view (never dips into `totalReserved`). Cron pre-filters
  out-of-fuel souls — bounds forever-cost and turns "keep this soul alive" into a public-good mechanic.
  Keeps Soul Evolution System (`stageOf`/`formIdOf`/`formSlugOf`/`evolutionOf` + Stage+Form in tokenURI)
  and audit hardening (`pendingRead` guard, non-reverting refund, owner `clearPending`, cost-gated evolution).
  Seeded 12 STT. Earlier 0xbc55… (+ cron 0x3cad…) / 0x92c5… / 0x5cc8… / 0x30e5… / 0x0b89… retired; STT
  recycled via `retireOld.ts`.
- **Live SoulprintCron:** `0x0bf4e395ad3746632f86b5254fa18f0db3479d95` — **autonomy criterion #4**
  (a fresh cron per redeploy: its `soulprint` is immutable). 30-min interval, batch 5, 40 STT,
  `subscriptionId` 2267508 armed; `ticks` accrues from 0 on this deploy. The mechanism (self-reschedule,
  no human tx) was proven live on prior crons `0x3cad…` (ticks 0→35, 2 real evolutions), `0x9eef…`
  (ticks 0→21+) and `0x9f4f…`; criterion holds. Owner can retune via `setParams`.
...
- **Dead/old cron:** `0xb7cc93f4…03cc` (0 STT, `subscriptionId` 0, points at retired Soulprint
  ...
  the live `0x0bf4…`; `recoverCron` keeps `0xb7cc…` as the historical recovery tool.
